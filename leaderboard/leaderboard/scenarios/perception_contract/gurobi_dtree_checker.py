...
class GurobiDTreeChecker(DTreeChecker):
    def __init__(self, npc_speeds: List[float] = [2.0, 15.0], pred_len: int = 2):
        ...
        self.cexes = [
            [48.4, 154.49914457742193, 39.48931292676345, 0.9682458365518543, 0.25, 153.920225739479, 39.83317788736895, 1.0, 0.0],
            [43.4, 163.92008864879608, 39.845357656478875, 1.0, 0.0, 163.92008864879608, 39.845357656478875, 1.0, 0.0],
            [53.4, 139.0219283637065, 35.72495290841897, 0.25, 0.9682458365518543, 139.91364580453788, 34.98849383355284, 0.998044963916957, 0.0625]
        ]
    ...
    def _create_model(self, pred_len: int = 2) -> gp.Model:
        ...
        # Add prediction variables for each frame
        for i in range(pred_len):
            model.addVar(lb=-GRB.INFINITY, name=f"x_{i}")
            model.addVar(lb=-GRB.INFINITY, name=f"y_{i}")
            model.addVar(lb=-1, ub=1, name=f"sin_yaw_{i}")
            model.addVar(lb=-1, ub=1, name=f"cos_yaw_{i}")
    ...
    def _add_npc_position_constraints(self, model: gp.Model, t: gp.Var, s: gp.Var, 
        ...
        for i, (npc_x, npc_y) in enumerate(npc_vars):
            ...
            time_diff = model.addVar(lb=-GRB.INFINITY, name=f"time_diff_{i}")
            ...
            # McCormick envelopes
            # self._add_mccormick_constraints(model, s, time_diff, displacement_x, 
            #                               self.npc_forward[0])
            # self._add_mccormick_constraints(model, s, time_diff, displacement_y, 
            #                               self.npc_forward[1])
    ...
    def _add_mccormick_constraints(self, model: gp.Model, x: gp.Var, y: gp.Var, 
                                xy: gp.Var, direction: float):
        """Add McCormick envelope constraints for bilinear term x*y*direction = xy"""
        # Assume bounds for x (speed) and y (time difference)
        x_lb, x_ub = self.npc_speeds[0], self.npc_speeds[1]
        y_lb, y_ub = 0, self.scenario_end_time - self.scenario_start_time
        
        # McCormick envelope constraints
        model.addConstr(xy >= direction * (x_lb * y + x * y_lb - x_lb * y_lb))
        model.addConstr(xy >= direction * (x_ub * y + x * y_ub - x_ub * y_ub))
        model.addConstr(xy <= direction * (x_ub * y + x * y_lb - x_ub * y_lb))
        model.addConstr(xy <= direction * (x_lb * y + x * y_ub - x_lb * y_ub))
    ...
    def find_counterexample_for_conjunct(self, conjunct: z3.BoolRef, pred_len: int = 2) -> Optional[List[float]]:
        """
        ...
        for i in range(pred_len):
            ego_state = {
                'x': model.getVarByName(f"x_{i}"),
                'y': model.getVarByName(f"y_{i}"),
                'sin_yaw': model.getVarByName(f"sin_yaw_{i}"),
                'cos_yaw': model.getVarByName(f"cos_yaw_{i}")
            }
            ego_states.append(ego_state)
            
            # Collision indicators
            ego_collision = model.addVar(vtype=GRB.BINARY, name=f"ego_collision_{i}")
            npc_collision = model.addVar(vtype=GRB.BINARY, name=f"npc_collision_{i}")
            
            collision_indicators.append(ego_collision)
            npc_collision_indicators.append(npc_collision)
        
        model.update()
        
        # Add collision detection constraints
        self._add_npc_position_constraints(model, t, s, npc_corners)
        self._add_ego_collision_constraints(model, collision_x, collision_y, 
                                          ego_states, collision_indicators)
